...
public class CablecarEntity extends Entity {
    ...
    private static final DataParameter<Boolean> BEHIND_CART = EntityDataManager.createKey(CablecarEntity.class, DataSerializers.BOOLEAN);
    ...
    protected void registerData() {
        ...
        dataManager.register(BEHIND_CART, false);
    }
    ...
    public void tick() {
        ...
        Block middle = world.getBlockState(this.getPosition()).getBlock();

        if(!(middle instanceof CablecarConnectable)) {
        ...
        List<CablecarEntity> cablecars = world.getEntitiesWithinAABB(CablecarEntity.class, this.getBoundingBox().offset(offsetX * 0.09, 0, offsetZ * 0.09), e -> e != this);
        if(!cablecars.isEmpty()) {
            this.dataManager.set(BEHIND_CART, true);
            return;
        } else if(this.dataManager.get(BEHIND_CART)) {
            cablecars = world.getEntitiesWithinAABB(CablecarEntity.class, this.getBoundingBox().offset(offsetX * 0.2, 0, offsetZ * 0.2), e -> e != this);
            if(cablecars.isEmpty()) {
                this.dataManager.set(BEHIND_CART, false);
            }
        }

        if(!this.dataManager.get(BEHIND_CART)) {
    ...
    protected void readAdditional(CompoundNBT compound) {
        ...
        this.dataManager.set(BEHIND_CART, compound.getBoolean("behind_cart"));
    }
    ...
    protected void writeAdditional(CompoundNBT compound) {
        ...
        compound.putBoolean("behind_cart", this.dataManager.get(BEHIND_CART));
    }
